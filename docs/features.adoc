...
== `preserve-order` (stabilized)

Object field order preservation during manifestification, upstream issue: https://github.com/google/jsonnet/issues/903

This feature is now stable and always available (no longer requires the `exp-preserve-order` compile-time feature gate).

It provides a CLI argument: `--preserve-order`,
...
Unfortunately, there is no integration with the `preserve-order` feature,
